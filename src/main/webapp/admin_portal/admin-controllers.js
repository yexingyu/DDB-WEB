...
.controller(
        ...
        [ '$scope', '$location', '$routeParams', 'ProductService',
                function($scope, $location, $routeParams, ProductService, MenuService) {
                    ...
                    $scope.product.total = parseFloat($scope.product.prices[1].value) + parseFloat($scope.product.fees[0].value) + parseFloat($scope.product.fees[1].value);
                    ...
                    $scope.product.principal = Math.round($scope.product.principal_raw*100)/100;
                    

                    
                    $scope.monthly_payment_raw =  $scope.monthly_interest_rate* $scope.product.principal_raw/(1 - Math.pow((1+$scope.monthly_interest_rate),-$scope.number_of_payments));
                    $scope.monthly_payment = Math.round($scope.monthly_payment_raw*100)/100;
                    
                    $scope.interest_raw = $scope.monthly_payment * $scope.number_of_payments - $scope.product.principal;
