 ...
 * product info
 */
public class StaplesCaPT {
  public static void main(String[] args) throws IOException {
    //set url
    String url = "http://www.staples.ca/en/Brother-HL-L2320D-Monochrome-Laser-Printer/product_1074269_2-CA_1_20001?externalize=certona";
    ...
    //description
    product_description_text = doc.select("div.copyBullets").get(0).text();

    //image
    product_image_text = doc.select("meta[property=\"og:image\"]").attr("content");

    //price
    product_price_text = doc.select("span.finalPrice").get(0).text().replace("*", "").replace("$", "");
