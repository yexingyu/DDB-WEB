 ...
 * product info
 */
public class BrownsShoesComPT {
  public static void main(String[] args) throws IOException {
    //set url
    String url = "http://www.brownsshoes.com/HARTVILLE/165713,default,pd.html";
    ...
    //name
    String productBrand;
    productBrand = doc.select("ol.breadcrumb").get(0).select("span.last").get(0).text();
    product_name_text = productBrand + " " + doc.select("h2.product-name").first().text();

    //description
    product_description_text = doc.select("div.product-main-attributes").first().text();

    //image
    product_image_text = doc.select("div#thumbnails").get(0).select("img").attr("src");

    //price
    product_price_text = doc.select("span.price-sales").get(0).text();
